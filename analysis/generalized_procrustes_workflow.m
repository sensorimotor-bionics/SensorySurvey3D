...
% got it such that we're only making comparisons with 2D annotations from 2025...
...
[~,three_dim_dorsum] = generalized_mesh_transform(mesh_2D,landmarks_2D,mesh_3D,landmarks_3D,"dorsal");

%% view annotations on flattened 3D mesh
[palmar_mask, ~] = get_hand_masks();
orig_size = size(palmar_mask);
[palmar,dorsal] = convert_3D_to_heatmap(three_dim,three_dim_dorsum,documented_electrodes,color_map,orig_size);
