...
## Protocol for Extraction of Custom Datasets
...
#### Experimental log columns ExperimentKey, Subject, and Session

For trawling purposes, the most important columns within the experiment log are ExperimentKey, Subject, and Session. If making use of general_data_extraction.m, data from individual subjects and sessions should be stored in individual folders in ```SensorySurvey3D\analysis\mesh_utils\[Subject]``` with names of the form ```[Subject].data.[Session]```. Individual .json annotation files generated by Survey3D should be stored inside the appropriate ```[Subject].data.[Session]``` folder with names of the form ```[ExperimentKey]*.json```. ExperimentKey, Subject, and Session identifiers can be any combination of letters and numbers and symbols such as _ and -.
...
#### Adding custom experimental log columns

All other columns in the experimental log are subject to change and should be tailored to your experimental needs. general_data_extraction.m can be modified for consistency with a custom experimental log by editing lines 44-46 and 63-65.

### Outputs of general_data_extraction.m

general_data_extraction.m saves individual .mat files for each row of the experimental log in ```SensorySurvey3D\analysis\External\[ExperimentKey]```. By default, .mat files produced by general_data_extraction.m for individual sessions and subjects have names of the form ```OLSData.[Subject].[Session].mat```. Summary .mat files produced by general_data_extraction.m for separate experiments have names of the form ```[ExperimentKey]Data_Recent.mat```. These files contain the aforementioned composite structures (e.g. Survey3DData), with variable names of the form ```[ExperimentKey]Data```.
