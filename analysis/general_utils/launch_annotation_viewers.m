function Survey3DDataRecord = launch_annotation_viewers(subject,Survey3DData,axis_alignment)
    ...
    for m = 1:length(which_models)
        ...
        chList = cell2mat([{Survey3DData.Ch1};{Survey3DData.Ch2};{Survey3DData.Ch3};{Survey3DData.Ch4}])';
        ...
        for q = 1:length(qualities)
            ...
            for ele = 1:length(unique_documented_electrodes)
                ...
                annotation_record.(qualities{q}).(this_ele) = struct();
                color_map.(qualities{q}).(this_ele) = struct();
        
                for ii = 1:length(which_rows)
                    session_set = ['s' Survey3DData(which_rows(ii)).Session '_' num2str(Survey3DData(which_rows(ii)).Set)];
                    annotation_record.(qualities{q}).(this_ele).(session_set) = [];
                    color_map.(qualities{q}).(this_ele).(session_set) = [];
                    for ns = 1:Survey3DData(which_rows(ii)).NumSense
                        if ~isempty(Survey3DData(which_rows(ii)).PFQualities(1,ns).(qualities{q}))
                            annotation_record.(qualities{q}).(this_ele).(session_set) =...
                                cat(2, annotation_record.(qualities{q}).(this_ele).(session_set) , Survey3DData(which_rows(ii)).PFBasics(1,ns).fields);
                            %annotation_record.(qualities{q}).(this_ele) = cat(2, annotation_record.(qualities{q}).(this_ele), Survey3DData(which_rows(ii)).PFBasics(1,ns).fields);
                        end
                    end
                    color_map.(qualities{q}).(this_ele).(session_set) = cat(2, color_map.(qualities{q}).(this_ele).(session_set), sum(annotation_record.(qualities{q}).(this_ele).(session_set),2));
                end      
                %color_map.(this_ele).(session_set) = cat(2, color_map.(this_ele).(session_set), sum(annotation_record.(qualities{q}).(this_ele).(session_set),2);
            end
        end
