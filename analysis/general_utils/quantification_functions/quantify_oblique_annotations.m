function Survey3DData = quantify_oblique_annotations(Survey3DData,MorphedMeshes)
    ...
    for idx = 1:length(Survey3DData)
        ...
        for f = 1:size(this_model.ThreeDim.faces,1)
            verts = this_model.ThreeDim.verts(this_model.ThreeDim.faces(f,:)+1,:);
            % magnitude of cross product is the positive area of the parallelogram having A and B as sides
            A = verts(2,:)-verts(1,:);
            B = verts(3,:)-verts(1,:);
            this_model.ThreeDim.face_area(f) = norm(cross(A,B))/2;
        end

        annotated_faces = sum(ismember(this_model.ThreeDim.faces,find(color_map{idx}>0)),2)==3;

        % sum areas of oblique faces which are annotated
        oblique_area = sum(this_model.ThreeDim.face_area(annotated_faces&this_model.ThreeDim.oblique));
        % sum areas of camera-facing faces which are annotated
        non_oblique_area = sum(this_model.ThreeDim.face_area(annotated_faces&~this_model.ThreeDim.oblique));
    ...
    % illustrate oblique faces
    % plot 3d mesh with alpha, color in faces which are oblique
    % figure;
    % shape_viewer(this_model.ThreeDim.verts_flat,this_model.ThreeDim.faces,this_model.ThreeDim.oblique,gca)
end